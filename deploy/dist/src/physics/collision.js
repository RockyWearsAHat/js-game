...
class CollisionSystem {
     ...
     */
    static checkCollision(position, objects, cameraDirection, playerVelocity, playerHeight = 1.7, playerRadius = 0.35) {
        const result = {
            isOnGround: false,
            isOnSlope: false,
            slopeAngle: 0,
        ...
        const capsuleTop = new THREE.Vector3(position.x, position.y + playerHeight, position.z);
        ...
        const groundDetectionPoints = [
            ...
            new THREE.Vector3(position.x + playerRadius * 0.7, position.y + 0.1, position.z + playerRadius * 0.7),
            new THREE.Vector3(position.x + playerRadius * 0.7, position.y + 0.1, position.z - playerRadius * 0.7),
            new THREE.Vector3(position.x - playerRadius * 0.7, position.y + 0.1, position.z + playerRadius * 0.7),
            new THREE.Vector3(position.x - playerRadius * 0.7, position.y + 0.1, position.z - playerRadius * 0.7),
        ];
        ...
        for (const rayStart of groundDetectionPoints) {
            const downRay = new THREE.Raycaster(rayStart, new THREE.Vector3(0, -1, 0), // Straight down
            0, 2.0 // Longer range to detect ground even when falling
            );
            ...
            if (downIntersects.length > 0) {
                ...
                if (Math.abs(rayStart.x - position.x) < 0.01 &&
        ...
        if (anyGroundDetected) {
            ...
            // Step up detection - more sophisticated
                ...
                const horizontalSpeed = new THREE.Vector3(playerVelocity.x, 0, playerVelocity.z).length();
        ...
        for (let i = 0; i < rayCount; i++) {
            ...
            horizontalRayDirections.push(new THREE.Vector3(Math.cos(angle), 0, Math.sin(angle)));
        }
        ...
        for (const rayHeight of rayHeights) {
            for (const dir of horizontalRayDirections) {
                const rayStart = new THREE.Vector3(position.x, position.y + rayHeight, position.z);
                ...
                if (intersects.length > 0) {
                    ...
                    if (hit.object === result.objectBelow &&
                    ...
                    if (hit.object.userData && hit.object.userData.isMovable) {
                        ...
                        const pushVector = dir
                            .clone()
                            .multiplyScalar(pushAmount *
                            Math.max(Math.abs(playerVelocity.x), Math.abs(playerVelocity.z)));
        ...
        this.checkForLedges(position, cameraDirection, objects, playerHeight, result);
        return result;
    }
    /**
     * Check for ledges that can be grabbed
     */
    static checkForLedges(position, lookDirection, objects, playerHeight, result) {
        ...
        const ledgeRay = new THREE.Raycaster(new THREE.Vector3(position.x + lookDirection.x * 0.3, grabHeight, position.z + lookDirection.z * 0.3), lookDirection, 0, 0.7 // Search distance
        );
        ...
        if (ledgeIntersects.length > 0) {
            ...
            if (hit.object.userData && hit.object.userData.nonClimbable)
                return;
            // Now check if there's space above the ledge
            const aboveRay = new THREE.Raycaster(new THREE.Vector3(hit.point.x - lookDirection.x * 0.1, hit.point.y + 0.5, // Check above the ledge
            hit.point.z - lookDirection.z * 0.1), new THREE.Vector3(0, -1, 0), // Look down
            0, 0.8 // Search distance
            );
